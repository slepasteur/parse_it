...
public:
  using base_iterator_type =
    typename std::conditional<std::is_same<Constness, const_view>::value, const std::byte*, std::byte*>::type;

  using iterator_type = typename std::conditional<
    std::is_same<Endianness, big_endian>::value, std::reverse_iterator<base_iterator_type>, base_iterator_type>::type;
