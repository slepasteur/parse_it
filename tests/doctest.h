...
struct DOCTEST_INTERFACE TestCase : public TestCaseData
{
  ...
  TestCase(
    funcType test, const char* file, unsigned line, const TestSuite& test_suite, const char* type = "",
    int template_id = -1);
  ...
  {                                                                                                                    \
    ...
    {                                                                                                                  \
      ...
      {                                                                                                                \
        ...
        res |= (isDebuggerActive() && !getContextOptions()->no_breaks) ? assertAction::dbgbreak : 0;                   \
...
template <int comparison, typename L, typename R>
DOCTEST_NOINLINE DOCTEST_FAST_ASSERT_RETURN_TYPE fast_binary_assert(
  assertType::Enum at, const char* file, int line, const char* expr, const DOCTEST_REF_WRAP(L) lhs,
...
fast_unary_assert(assertType::Enum at, const char* file, int line, const char* expr, const DOCTEST_REF_WRAP(L) val)
{
  bool failed = !val;
...
public:
  ...
  {
    static_assert(
      deferred_false<T>::value, "Cannot pass temporaries or rvalues to the streaming operator because it "
...
#define DOCTEST_REGISTER_TYPED_TEST_CASE_IMPL(func, type, decorators, idx)                                             \
  doctest::detail::regTest(                                                                                            \
    doctest::detail::TestCase(                                                                                         \
      func, __FILE__, __LINE__, doctest_detail_test_suite_ns::getCurrentTestSuite(),                                   \
...
#define DOCTEST_SUBCASE(name)                                                                                          \
  if (                                                                                                                 \
    const doctest::detail::Subcase & DOCTEST_ANONYMOUS(_DOCTEST_ANON_SUBCASE_) DOCTEST_UNUSED =                        \
...
#define DOCTEST_FAST_BINARY_ASSERT(assert_type, comparison, ...)                                                       \
  do                                                                                                                   \
  {                                                                                                                    \
    int _DOCTEST_FAST_RES = doctest::detail::fast_binary_assert<doctest::detail::binaryAssertComparison::comparison>(  \
...
#define DOCTEST_FAST_UNARY_ASSERT(assert_type, ...)                                                                    \
  doctest::detail::fast_unary_assert(doctest::assertType::assert_type, __FILE__, __LINE__, #__VA_ARGS__, __VA_ARGS__)
...
bool checkIfShouldThrow(assertType::Enum at)
{
  ...
  if (
    (at & assertType::is_check) //!OCLINT bitwise operator in conditional
    && getContextOptions()->abort_after > 0
    && (g_cs->numAssertsFailed + g_cs->numAssertsFailedForCurrentTestCase_atomic) >= getContextOptions()->abort_after)
    return true;
...
{
  if (
    std::find(getExceptionTranslators().begin(), getExceptionTranslators().end(), et)
...
bool parseOption(
  int argc, const char* const* argv, const char* pattern, String& res, const String& defaultVal = String())
{
  res = defaultVal;
#ifndef DOCTEST_CONFIG_NO_UNPREFIXED_OPTIONS
...
#define DOCTEST_PARSE_AS_BOOL_OR_FLAG(name, sname, var, default)                                                       \
  if (                                                                                                                 \
    parseIntOption(argc, argv, name "=", option_bool, intRes)                                                          \
...
#define DOCTEST_PARSE_INT_OPTION(name, sname, var, default)                                                            \
  if (                                                                                                                 \
    parseIntOption(argc, argv, name "=", option_int, intRes)                                                           \
...
#define DOCTEST_PARSE_STR_OPTION(name, sname, var, default)                                                            \
  if (                                                                                                                 \
    parseOption(argc, argv, name "=", strRes, default) || parseOption(argc, argv, sname "=", strRes, default)          \
    || withDefaults)                                                                                                   \
  p->var = strRes
...
int Context::run()
{
  ...
  for (auto& curr : testArray)
  {
    ...
    // execute the test if it passes all the filtering
    {
      ...
      if (
        Approx(p->currentTest->m_timeout).epsilon(DBL_EPSILON) != 0
